...
// Usar ANON_KEY para aceitar tokens de usuários autenticados
...
serve(async (req) => {
  ...
  try {
    ...
    // Decodificar JWT para extrair user_id (sem validar assinatura)
    let userId: string;
    try {
      ...
      if (!userId) {
        return new Response(
          JSON.stringify({ error: "Token inválido: sub não encontrado" }),
          { status: 401 }
        );
      }
    } catch (error) {
    ...
    // Criar cliente Supabase com o token do usuário para RLS
    ...
    // Validar que o cirurgião é o proprietário do pedido (RLS será aplicado automaticamente)
    const { data: pedido, error: pedidoError } = await supabaseWithAuth
    ...
    if (pedidoError || !pedido) {
      return new Response(
        JSON.stringify({ error: "Pedido não encontrado ou sem permissão" }),
        { status: 404 }
      );
    }

    if (pedido.surgeon_id !== userId) {
      return new Response(
        JSON.stringify({
    ...
    const { data: existingDoc } = await supabaseWithAuth
    ...
    // Upload para Storage (usar cliente com auth)
    ...
    // Registrar na tabela pedidos_documentos (usar cliente com auth para RLS)
    const { data: documento, error: dbError } = await supabaseWithAuth
      .from("pedidos_documentos")
      .insert({
        pedido_id: pedidoId,
        ...
        notas: `Upload realizado por ${userId} em ${new Date().toISOString()}`,
      })
      .select()
      .single();
    ...
    if (dbError) {
      ...
      await supabaseWithAuth.storage.from("pedidos-documentos").remove([storagePath]);
      return new Response(
        JSON.stringify({
    ...
    // Registrar na auditoria (usar cliente com auth)
    await supabaseWithAuth.from("audit_log").insert({
      pedido_id: pedidoId,
      changed_by: userId,
      status_from: null,
      status_to: null,
