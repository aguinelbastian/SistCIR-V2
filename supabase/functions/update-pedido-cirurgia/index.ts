...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!usuario_id) {
      return new Response(JSON.stringify({ success: false, message: 'Usuário não autenticado' }), { status: 401, headers: corsHeaders });
    }
    ...
    if (!id) {
      return new Response(JSON.stringify({ success: false, message: 'ID do pedido é obrigatório' }), { status: 400, headers: corsHeaders });
    }
    ...
    if (novoEstado === '7_AGENDADO_CC') {
      const { data: validacao, error: valError } = await supabase.rpc('validate_pinças_before_scheduling', { p_pedido_id: id });
      if (valError) throw valError;
      if (validacao && validacao.length > 0 && !validacao[0].valid) {
        return new Response(JSON.stringify({ success: false, message: validacao[0].message }), { status: 200, headers: corsHeaders });
      }
    }
    ...
    if (pedidoCompleto) {
      ...
      if ((calendarAction === 'CREATE' || calendarAction === 'UPDATE') && !pedidoCompleto.scheduled_date) {
        calendarAction = 'NONE';
      }
      ...
      if (novoEstado && novoEstado !== estadoAtual) {
        ...
        if (botToken) {
          promises.push(notifyTelegramBySector(pedidoCompleto, novoEstado, botToken).then(r => ({ type: 'telegram', result: r })));
        }
      }

      if (calendarAction !== 'NONE') {
        promises.push(syncGoogleCalendar(supabase, pedidoCompleto, calendarAction).then(r => ({ type: 'calendar', result: r })));
      }
    }
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: novoEstado ? `Transição para ${novoEstado} realizada com sucesso` : 'Pedido atualizado com sucesso',
      data: updatedRecord,
      notificationSent,
      notificationWarning,
      notifiedGroups,
