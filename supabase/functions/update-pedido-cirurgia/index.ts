...
type Estado = '1_RASCUNHO' | '2_AGUARDANDO_OPME' | '3_EM_AUDITORIA' | '4_PENDENCIA_TECNICA' | '5_AUTORIZADO' | '6_AGUARDANDO_ALOCACAO' | '7_AGENDADO_CC' | '8_EM_EXECUCAO' | '9_REALIZADO' | '10_CANCELADO';
...
const transicoesPermitidas: Record<Estado, Estado[]> = {
  ...
  '4_PENDENCIA_TECNICA': ['5_AUTORIZADO', '3_EM_AUDITORIA', '10_CANCELADO'],
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const tipoPendencia = body.tipo_pendencia;
    const descricaoPendencia = body.descricao_pendencia;
    const updates = Object.fromEntries(
      Object.entries(body).filter(([key]) => !['id', 'status', 'pedido_id', 'new_status', 'tipo_pendencia', 'descricao_pendencia', 'notes'].includes(key))
    );
    ...
    // Se transição para Status 4 (Pendência Técnica), criar registro em pedidos_pendencias
    if (novoEstado === '4_PENDENCIA_TECNICA' && tipoPendencia && descricaoPendencia) {
      const { error: pendenciaError } = await supabase.from('pedidos_pendencias').insert({
        pedido_id: id,
        tipo_pendencia: tipoPendencia,
        descricao: descricaoPendencia,
        setor_responsavel: tipoPendencia === 'INSUMOS_OPME' ? 'OPME' : 'FINANCEIRO',
        criado_por: usuario_id,
      });

      if (pendenciaError) {
        console.error('Pendência insert error:', pendenciaError);
      }
    }

    // Se transição para Status 5 (Autorizado) a partir de Status 4, marcar pendência como resolvida
    if (novoEstado === '5_AUTORIZADO' && estadoAtual === '4_PENDENCIA_TECNICA') {
      const { error: updatePendenciaError } = await supabase
      ...
      if (updatePendenciaError) {
        console.error('Pendência update error:', updatePendenciaError);
      }
    }
