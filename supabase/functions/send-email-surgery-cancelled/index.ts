...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (resendApiKey) {
      const htmlContent = `
        <h2 style="color: #cc0000;">SistCIR — Cirurgia Cancelada</h2>
        <p>Olá, <strong>${surgeonName}</strong>.</p>
        <p>A solicitação de cirurgia abaixo foi <strong>cancelada</strong>.</p>
        <ul>
        ...
        <br/>
        <p><a href="${magicLink}" style="background-color: #cc0000; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block;">Acompanhar Status</a></p>
        <br/>
      ...
      const emailRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to: surgeon.email,
          subject: 'SistCIR — Cirurgia Cancelada',
          html: htmlContent,
        }),
      })
