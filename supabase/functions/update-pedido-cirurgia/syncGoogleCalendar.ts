...
export async function syncGoogleCalendar(supabase: any, pedido: any, action: 'CREATE' | 'UPDATE' | 'DELETE') {
  ...
  const refreshOAuthToken = async (refreshToken: string, clientId: string, clientSecret: string) => {
    const res = await fetch('https://oauth2.googleapis.com/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
  ...
  const performSync = async (type: 'SERVICE_ACCOUNT' | 'PERSONAL', token: string, calendarId: string, surgeonId: string | null) => {
    let query = supabase.from('pedidos_calendar_events').select('*').eq('pedido_id', pedido.id).eq('calendar_type', type);
    ...
    try {
      if (action === 'CREATE' || (action === 'UPDATE' && !existing)) {
        ...
        const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events/${existing.google_event_id}`, {
          method: 'PUT',
          headers,
          body: JSON.stringify(payload)
        });
        ...
        await supabase.from('pedidos_calendar_events')
          .update({
      ...
      const errorMsg = isAuthError ? 'Sincronização com calendário pessoal expirou. Reautentique.' : e.message;
      
      warnings.push(`Erro no calendário ${type}: ${errorMsg}`);
      
      if (existing) {
        await supabase.from('pedidos_calendar_events').update({ 
          sync_status: 'FAILED', 
          sync_error: errorMsg 
        }).eq('id', existing.id);
  ...
  if (refreshToken && clientId && clientSecret) {
    try {
      ...
      await supabase.from('pedidos_calendar_events')
        .update({ sync_status: 'FAILED', sync_error: 'Sincronização com calendário pessoal expirou. Reautentique.' })
