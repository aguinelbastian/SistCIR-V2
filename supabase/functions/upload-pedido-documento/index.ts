...
const DOCUMENTO_OBRIGATORIO_POR_PROCEDIMENTO: Record<string, string[]> = {
  PROSTATECTOMIA_RADICAL_ROBOTICA: ['JUSTIFICATIVA_CLINICA', 'EXAME_COMPLEMENTAR'],
  NEFRECTOMIA_PARCIAL_ROBOTICA: ['JUSTIFICATIVA_CLINICA', 'EXAME_COMPLEMENTAR'],
...
serve(async (req) => {
  ...
  try {
    ...
    if (!pedidoId) {
      return new Response(JSON.stringify({ error: 'pedido_id não fornecido' }), { status: 400 })
    }

    if (!documentoTipo) {
      return new Response(JSON.stringify({ error: 'documento_tipo não fornecido' }), {
        status: 400,
      })
    }
