...
export default function ResourceAllocationCreate() {
  ...
  useEffect(() => {
    Promise.all([
      supabase
        ...
        .select('id, patients(full_name), surgeon_id, previsao_tempo_minutos, status')
    ...
    supabase
      ...
      .then(({ data: prefs }) => {
        ...
        if (prefs && prefs.length > 0) {
          form.setValue('is_fallback_allocation', false)
        } else {
          form.setValue('is_fallback_allocation', true)
        }
        form.setValue('surgical_block_id', '') // reset block
      })

    const pedido = data.pedidos.find((p) => p.id === selectedPedidoId)
    if (pedido) {
      if (pedido.surgeon_id) form.setValue('allocated_surgeon_id', pedido.surgeon_id)
      if (pedido.previsao_tempo_minutos)
        form.setValue('estimated_duration_minutes', pedido.previsao_tempo_minutos)
    }
  }, [selectedPedidoId, data.pedidos, form])
  ...
  }, [selectedBlockId, isFallback, preferences, data.blocks, form])
  ...
  const onSubmit = async (values: ResourceAllocationFormValues) => {
    ...
    try {
      await executeWithRetry(async () => {
        return await supabase.from('resource_allocation').insert(payload)
      })
      toast({ title: 'Sucesso', description: 'Cirurgia alocada com sucesso!' })
      navigate('/alocacao-recursos')
    } catch (error: any) {
      toast({
        title: 'Erro',
        description: error.message || 'Falha na alocação.',
        variant: 'destructive',
      })
    }
  }
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-2">
                      <FormLabel>Pedido Cirúrgico (Aprovados / Aguardando Mapa)</FormLabel>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-2">
                      <FormLabel>
                        Bloco Cirúrgico{' '}
                        {isFallback ? '(Todos os Disponíveis)' : '(Preferências do Cirurgião)'}
                      </FormLabel>
