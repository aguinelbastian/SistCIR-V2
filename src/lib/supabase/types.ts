  ...
  public: {
    Tables: {
      ...
      opme_items: {
        ...
        Relationships: [
          {
            foreignKeyName: "opme_items_catalog_id_fkey"
            columns: ["catalog_id"]
            isOneToOne: false
            referencedRelation: "opme_catalog"
            referencedColumns: ["id"]
          },
        ]
      }
      ...
      pedido_opme_consumo: {
        ...
        Relationships: [
          {
            foreignKeyName: "pedido_opme_consumo_opme_item_id_fkey"
          ...
          {
            foreignKeyName: "pedido_opme_consumo_pedido_id_fkey"
            columns: ["pedido_id"]
            isOneToOne: false
            referencedRelation: "mv_kpi_cirurgias"
            referencedColumns: ["id"]
      ...
      pedido_opme_items: {
        ...
        Relationships: [
          ...
          {
            foreignKeyName: "pedido_opme_items_catalog_id_fkey"
            columns: ["catalog_id"]
            isOneToOne: false
            referencedRelation: "opme_catalog"
            referencedColumns: ["id"]
      ...
      pedidos_pendencias: {
        ...
        Relationships: [
          {
            foreignKeyName: "pedidos_pendencias_criado_por_fkey"
            columns: ["criado_por"]
            isOneToOne: false
            referencedRelation: "profiles"
            referencedColumns: ["id"]
          ...
          {
            foreignKeyName: "pedidos_pendencias_pedido_id_fkey"
            columns: ["pedido_id"]
            isOneToOne: false
            referencedRelation: "mv_kpi_cirurgias"
            referencedColumns: ["id"]
          ...
          {
            foreignKeyName: "pedidos_pendencias_resolvido_por_fkey"
...
//   resolvido_em: timestamp without time zone (nullable)
//   resolvido_por: uuid (nullable)
//   created_at: timestamp without time zone (nullable, default: now())
//   updated_at: timestamp without time zone (nullable, default: now())
// Table: procedures
//   id: uuid (not null, default: uuid_generate_v4())
...
// Table: opme_catalog
//   PRIMARY KEY opme_catalog_pkey: PRIMARY KEY (id)
// Table: opme_items
//   FOREIGN KEY opme_items_catalog_id_fkey: FOREIGN KEY (catalog_id) REFERENCES opme_catalog(id)
...
// Table: pedido_opme_consumo
//   FOREIGN KEY pedido_opme_consumo_opme_item_id_fkey: FOREIGN KEY (opme_item_id) REFERENCES opme_items(id)
//   FOREIGN KEY pedido_opme_consumo_pedido_id_fkey: FOREIGN KEY (pedido_id) REFERENCES pedidos_cirurgia(id) ON DELETE CASCADE
//   PRIMARY KEY pedido_opme_consumo_pkey: PRIMARY KEY (id)
//   FOREIGN KEY pedido_opme_consumo_registered_by_fkey: FOREIGN KEY (registered_by) REFERENCES auth.users(id)
// Table: pedido_opme_items
//   FOREIGN KEY pedido_opme_items_added_by_fkey: FOREIGN KEY (added_by) REFERENCES profiles(id)
//   FOREIGN KEY pedido_opme_items_catalog_id_fkey: FOREIGN KEY (catalog_id) REFERENCES opme_catalog(id)
...
// Table: pedidos_pendencias
//   FOREIGN KEY pedidos_pendencias_criado_por_fkey: FOREIGN KEY (criado_por) REFERENCES profiles(id)
//   FOREIGN KEY pedidos_pendencias_pedido_id_fkey: FOREIGN KEY (pedido_id) REFERENCES pedidos_cirurgia(id) ON DELETE CASCADE
//   PRIMARY KEY pedidos_pendencias_pkey: PRIMARY KEY (id)
//   FOREIGN KEY pedidos_pendencias_resolvido_por_fkey: FOREIGN KEY (resolvido_por) REFERENCES profiles(id)
//   CHECK pedidos_pendencias_setor_responsavel_check: CHECK ((setor_responsavel = ANY (ARRAY['FINANCEIRO'::text, 'OPME'::text])))
//   CHECK pedidos_pendencias_tipo_pendencia_check: CHECK ((tipo_pendencia = ANY (ARRAY['ORCAMENTO_PACIENTE'::text, 'AUTORIZACAO_CONVENIO'::text, 'INSUMOS_OPME'::text])))
// Table: procedures
...
// Table: opme_catalog
//   Policy "opme_catalog_select_policy" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: opme_items
...
//     USING: (EXISTS ( SELECT 1    FROM user_roles ur   WHERE ((ur.user_id = auth.uid()) AND (ur.role = ANY (ARRAY['opme'::user_role_type, 'admin'::user_role_type])) AND (ur.is_active = true))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM user_roles ur   WHERE ((ur.user_id = auth.uid()) AND (ur.role = ANY (ARRAY['opme'::user_role_type, 'admin'::user_role_type])) AND (ur.is_active = true))))
//   Policy "pedido_opme_items_delete" (DELETE, PERMISSIVE) roles={public}
//     USING: ((EXISTS ( SELECT 1    FROM pedidos_cirurgia pc   WHERE ((pc.id = pedido_opme_items.pedido_id) AND (pc.surgeon_id = auth.uid()) AND (pc.status = '1_RASCUNHO'::surgery_status)))) OR has_role('admin'::text))
//   Policy "pedido_opme_items_insert" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: ((EXISTS ( SELECT 1    FROM pedidos_cirurgia pc   WHERE ((pc.id = pedido_opme_items.pedido_id) AND (pc.surgeon_id = auth.uid()) AND (pc.status = '1_RASCUNHO'::surgery_status)))) OR has_role('admin'::text))
//   Policy "pedido_opme_items_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "pedido_opme_items_update" (UPDATE, PERMISSIVE) roles={public}
//     USING: ((EXISTS ( SELECT 1    FROM pedidos_cirurgia pc   WHERE ((pc.id = pedido_opme_items.pedido_id) AND (pc.surgeon_id = auth.uid()) AND (pc.status = '1_RASCUNHO'::surgery_status)))) OR has_role('admin'::text))
// Table: pedidos_calendar_events
...
//     USING: (has_role('coordinator'::text) AND (status = ANY (ARRAY['5_AUTORIZADO'::surgery_status, '6_AGUARDANDO_ALOCACAO'::surgery_status, '7_AGENDADO_CC'::surgery_status])))
...
//     USING: (has_role('opme'::text) AND (status = ANY (ARRAY['2_AGUARDANDO_OPME'::surgery_status, '5_AUTORIZADO'::surgery_status, '6_AGUARDANDO_ALOCACAO'::surgery_status, '7_AGENDADO_CC'::surgery_status, '8_EM_EXECUCAO'::surgery_status])))
...
// Table: pedidos_pendencias
//   Policy "Financeiro e OPME podem atualizar pendências" (UPDATE, PERMISSIVE) roles={public}
//     USING: ((auth.jwt() ->> 'role'::text) = ANY (ARRAY['financeiro'::text, 'opme'::text, 'admin'::text]))
//   Policy "Financeiro e OPME podem criar pendências" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: ((auth.jwt() ->> 'role'::text) = ANY (ARRAY['financeiro'::text, 'opme'::text, 'admin'::text]))
//   Policy "Financeiro e OPME podem ler pendências" (SELECT, PERMISSIVE) roles={public}
//     USING: ((auth.jwt() ->> 'role'::text) = ANY (ARRAY['financeiro'::text, 'opme'::text, 'admin'::text]))
// Table: procedures
...
//     IF NEW.status::TEXT = '7_AGENDADO_CC' AND OLD.status::TEXT != '7_AGENDADO_CC' THEN
//       SELECT * INTO v_validation_result
//       FROM validate_pinças_before_scheduling(NEW.id);
//   
//       IF NOT v_validation_result.valid THEN
//         RAISE EXCEPTION 'Validação de pinças falhou: %', v_validation_result.message;
//       END IF;
//     END IF;
//   
//     -- Registrar auditoria em pedidos_cirurgia_auditoria
//     INSERT INTO pedidos_cirurgia_auditoria (pedido_id, acao, campos_alterados, usuario_id)
//     VALUES (
//       NEW.id,
//       'TRANSICAO_ESTADO: ' || COALESCE(OLD.status::TEXT, 'NULL') || ' → ' || NEW.status::TEXT,
...
//   $function$
//   
// FUNCTION fn_cancel_pedido_restore_opme()
//   CREATE OR REPLACE FUNCTION public.fn_cancel_pedido_restore_opme()
//    RETURNS trigger
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
//   BEGIN
//     -- Verifica se o status mudou especificamente para CANCELADO
//     IF NEW.status = '10_CANCELADO'::surgery_status AND OLD.status != '10_CANCELADO'::surgery_status THEN
//       
//       -- Deleta as alocações físicas (Gasto de Sala) deste pedido.
//       -- NOTA ARQUITETURAL: Ao deletar aqui, o gatilho 'trg_restore_pinca_lives' 
//       -- que criamos anteriormente na tabela 'pedido_opme_consumo' será acionado 
//       -- automaticamente para cada linha deletada, devolvendo as vidas ao estoque global.
//       DELETE FROM pedido_opme_consumo 
...
//   $function$
//   
// FUNCTION fn_decrement_pinca_lives()
//   CREATE OR REPLACE FUNCTION public.fn_decrement_pinca_lives()
//    RETURNS trigger
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
//   BEGIN
//     UPDATE opme_items
//     SET current_lives = current_lives - NEW.quantity
//     WHERE id = NEW.opme_item_id AND item_type = 'pinça_clicada';
//     RETURN NEW;
//   END;
//   $function$
//   
...
//   $function$
//   
// FUNCTION fn_restore_pinca_lives()
//   CREATE OR REPLACE FUNCTION public.fn_restore_pinca_lives()
//    RETURNS trigger
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
//   BEGIN
//     UPDATE opme_items
//     SET current_lives = current_lives + OLD.quantity
//     WHERE id = OLD.opme_item_id AND item_type = 'pinça_clicada';
//     RETURN OLD;
//   END;
//   $function$
//   
...
//   $function$
//   
// FUNCTION is_valid_transition(text, text)
...
//       WHEN p_current_status = '5_AUTORIZADO' AND p_new_status = '6_AGUARDANDO_MAPA' THEN TRUE
//       WHEN p_current_status = '6_AGUARDANDO_MAPA' AND p_new_status = '7_AGENDADO_CC' THEN TRUE
...
//       WHEN p_current_status IN ('1_RASCUNHO', '2_AGUARDANDO_OPME', '3_EM_AUDITORIA', '4_PENDENCIA_TECNICA', '5_AUTORIZADO', '6_AGUARDANDO_MAPA', '7_AGENDADO_CC', '8_EM_EXECUCAO', '9_REALIZADO') AND p_new_status = '10_CANCELADO' THEN TRUE
//       ELSE FALSE
//     END;
//   END;
//   $function$
...
//     SELECT status::TEXT, surgeon_id INTO v_current_status, v_surgeon_id
...
//     END IF;
//   
//     -- Atualizar status com cast correto para ENUM
//     UPDATE pedidos_cirurgia
//     SET 
//       status = p_new_status::surgery_status,
//       updated_at = NOW()
//     WHERE id = p_pedido_id;
//   
//     -- Registrar auditoria com estrutura correta (acao + campos_alterados)
//     INSERT INTO pedidos_cirurgia_auditoria (pedido_id, acao, campos_alterados, usuario_id)
//     VALUES (
//       p_pedido_id, 
//       'TRANSICAO_ESTADO: ' || v_current_status || ' → ' || p_new_status,
//       ARRAY['status', 'updated_at'],
//       v_current_user_id
//     );
//   
...
//   DECLARE
//     v_req RECORD;
//     v_total_lives_in_inventory INT;
//   BEGIN
//     -- Iterar sobre cada item do CATÁLOGO solicitado no pedido
//     FOR v_req IN
//       SELECT 
//         oc.id as catalog_id,
//         oc.name,
//         oc.item_type,
//         SUM(poi.quantity) as total_solicitado
//       FROM pedido_opme_items poi
//       JOIN opme_catalog oc ON poi.catalog_id = oc.id
//       WHERE poi.pedido_id = p_pedido_id
//       GROUP BY oc.id, oc.name, oc.item_type
//     LOOP
//       -- Se é pinça clicada, validar se o hospital tem vidas suficientes no inventário global
//       IF v_req.item_type = 'pinça_clicada' THEN
//         
//         SELECT COALESCE(SUM(current_lives), 0) INTO v_total_lives_in_inventory
//         FROM opme_items
//         WHERE catalog_id = v_req.catalog_id AND is_active = true;
//   
//         -- Validar se há vidas suficientes no estoque global
//         IF v_total_lives_in_inventory < v_req.total_solicitado THEN
//           RETURN QUERY SELECT 
//             FALSE, 
//             'Estoque global insuficiente para ' || v_req.name || '. Vidas disponíveis: ' || v_total_lives_in_inventory::TEXT || ', Solicitadas: ' || v_req.total_solicitado::TEXT;
//           RETURN;
//         END IF;
//       END IF;
//     END LOOP;
//   
//     RETURN QUERY SELECT TRUE, 'Estoque global de pinças validado com sucesso para agendamento';
//   END;
//   $function$
//   
...
// Table: pedido_opme_consumo
//   trg_decrement_pinca_lives: CREATE TRIGGER trg_decrement_pinca_lives AFTER INSERT ON public.pedido_opme_consumo FOR EACH ROW EXECUTE FUNCTION fn_decrement_pinca_lives()
//   trg_restore_pinca_lives: CREATE TRIGGER trg_restore_pinca_lives AFTER DELETE ON public.pedido_opme_consumo FOR EACH ROW EXECUTE FUNCTION fn_restore_pinca_lives()
// Table: pedidos_cirurgia
...
//   trg_cancel_pedido_restore_opme: CREATE TRIGGER trg_cancel_pedido_restore_opme AFTER UPDATE OF status ON public.pedidos_cirurgia FOR EACH ROW EXECUTE FUNCTION fn_cancel_pedido_restore_opme()
...
// Table: pedidos_pendencias
//   CREATE INDEX idx_pedidos_pendencias_pedido_id ON public.pedidos_pendencias USING btree (pedido_id)
//   CREATE INDEX idx_pedidos_pendencias_resolvido_em ON public.pedidos_pendencias USING btree (resolvido_em)
//   CREATE INDEX idx_pedidos_pendencias_setor ON public.pedidos_pendencias USING btree (setor_responsavel)
// Table: procedures
